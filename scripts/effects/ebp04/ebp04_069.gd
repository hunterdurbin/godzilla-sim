...
## <Awakening 4> If this card is in the same column as your opponent’s blue monster
## card, this card gains +5000 counter power. (Active if your monster card is in zone 4
## or beyond.)
##
## Tested: Yes
## Known issues: None
...
func get_counter_power_modifier(ctx: EffectContext) -> int:
	if not ctx.is_awakening(4):
		return 0
	if not CardUtils.has_color(ctx.opponent.current_monster, CardEnums.CardColor.BLUE):
		return 0

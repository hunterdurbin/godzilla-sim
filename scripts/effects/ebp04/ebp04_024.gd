...
func on_enter(ctx: EffectContext) -> void:
	...
	while true:
		...
		if total_rank <= 7 or opp_zones_with_cards.is_empty():
			break

		var chosen: int = await ctx.effect_handler.select_zone_target(
			ctx.owner.player_id, ctx.opponent.player_id, opp_zones_with_cards,
			"Destroy an opponent's battle card (total remaining ranks must reach 7 or less, or skip):",
			true)
		if chosen < 0:
			break

...
func _invade(hand_index: int, state: GameState) -> void:
	...
	# Collect monster advance and invasion observed entries once for the entire invasion
	if effect_handler and player.monster_zone > start_zone:
		deferred_entries.append_array(effect_handler.collect_monster_advance_entries(player.player_id, start_zone, player.monster_zone))

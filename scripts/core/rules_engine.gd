...
##
## Effect-driven adjustments (rank modifiers, blocked zones, play
## restrictions, ...) come from the injected EffectQueries — the read-only
## aggregation layer over active card effects. `queries` must be bound to the
## same GameState the validation methods receive; TurnManager wires it from
## EffectHandler.queries. Cards without effect scripts contribute nothing,
## so a state without effects behaves identically to the base rules.
...
func get_playable_battle_cards(player: PlayerState, opponent: PlayerState) -> Array[int]:
	...
	for i in range(player.hand.size()):
		...
		if card.get("card_type") == CardEnums.CardType.BATTLE:
			...
			if not queries.can_card_be_played(player.player_id, card):
				continue
			var base_rank: int = card.get("rank", 99)
			base_rank += queries.get_play_rank_modifier(player.player_id, card)
			if base_rank <= opponent.monster_zone:
				indices.append(i)
			else:
				# Check if any zone provides enough rank reduction
				for zi in valid_zones:
					var zone_mod: int = queries.get_zone_play_rank_modifier(player.player_id, card, zi)
...
func get_valid_zones_for_battle_card(player: PlayerState, opponent: PlayerState) -> Array[int]:
	...
	var valid: Array[int] = []
	var blocked: Array[int] = queries.get_opponent_blocked_zones(opponent.player_id)
	for i in range(8):
...
func can_play_battle_card_at_zone(card: Dictionary, zone_index: int, player: PlayerState, opponent: PlayerState) -> bool:
	...
	effective_rank += queries.get_play_rank_modifier(player.player_id, card)
	effective_rank += queries.get_zone_play_rank_modifier(player.player_id, card, zone_index)
	...
	if zone_index in blocked:
		return false
	var required: Array[int] = queries.get_card_required_play_zones(player.player_id, card)
...
func get_playable_strategy_cards(player: PlayerState) -> Array[int]:
	...
	if queries.are_opponent_strategy_plays_blocked(player.player_id):
		return indices
	for i in range(player.hand.size()):
		var card: Dictionary = player.hand[i]
		if card.get("card_type") == CardEnums.CardType.STRATEGY:
			var effective_rank: int = card.get("rank", 99)
			effective_rank += queries.get_play_rank_modifier(player.player_id, card)
			effective_rank += queries.get_strategy_hand_rank_modifier(player.player_id, card)
...
func get_discardable_cards_for_invade(player: PlayerState, opponent: PlayerState) -> Array[int]:
	...
	if queries.is_own_invasion_blocked(player.player_id):
		return indices
	if queries.is_invasion_blocked(opponent.player_id):
		return indices
...
func get_playable_monsters(player: PlayerState) -> Array[int]:
	...
	for i in range(player.hand.size()):
		...
		if queries.get_burst_rank(card) == cur_rank:
			indices.append(i)
			continue
		# Alternate play cost (e.g. EBP04-012 Biollante Plant Beast Form)
		if queries.can_monster_be_played_from_hand(player.player_id, card):
			indices.append(i)
	return indices

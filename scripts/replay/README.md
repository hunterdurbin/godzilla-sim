...
| `game_log.gd` | `GameLog` — structured log tokens (static constructors like `played_battle`, `invaded`, `counter_succeeded`) + locale-aware BBCode `render()` so each client renders the shared log in its own language; also chat BBCode. Loads effect icons from `res://assets/effectIcons/<en\|ja>/…` |

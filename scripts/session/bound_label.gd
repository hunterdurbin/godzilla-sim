...
func _try_bind() -> void:
	...
	# Client peers rebuild their PlayerState objects on every state receive,
	# so signal subscriptions alone go stale — rebind + refresh per apply.
	if not _session.is_running():
		if not _session.client_state_applied.is_connected(_on_client_state_applied):
			_session.client_state_applied.connect(_on_client_state_applied)
...
func _on_client_state_applied() -> void:
	_bind_player()
	_on_signal_fired()
...
func _disconnect_all() -> void:
	_disconnect_player()
	if _session and _session.client_state_applied.is_connected(_on_client_state_applied):
		_session.client_state_applied.disconnect(_on_client_state_applied)

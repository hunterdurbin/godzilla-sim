...
func _bind(session: GameSession) -> void:
	...
	if _bound_player == p:
		return
	if _bound_player and _bound_player.deck_changed.is_connected(_refresh):
		_bound_player.deck_changed.disconnect(_refresh)
	_bound_player = p
	p.deck_changed.connect(_refresh)
	_refresh()


func _refresh() -> void:
	if _bound_player:
		text = format_string % _bound_player.main_deck.size()
